...
fqc_path = os.path.join('..','data','FQC')
oqc_path = os.path.join('..','data','OQC')
options = EdgeOptions()
...
def fqc_crawl(driver,start_date,end_date):
...
def oqc_crawl(driver,start_date,end_date):
...
def do_crawl():
    ...
    fqc_start,oqc_start = get_start_date()
    start = min(fqc_start,oqc_start)
    end = fqc_start+timedelta(days=6)
    if start < end_date:
        if end > end_date:
            end = end_date
        while True:
            ...
            while check_status != 2:
                try:
                    ...
                    check_status += oqc_crawl(driver,start.strftime('%Y/%m/%d'),end)
                    check_status += fqc_crawl(driver,start.strftime('%Y/%m/%d'),end)
